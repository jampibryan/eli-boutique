    ...
    @else
                        ...
                        @foreach($carrito as $index => $item)
                            @php
                                ...
                                foreach($carrito as $c) {
                                    if ($c['producto_id'] == $item['producto_id'] && $c['talla_id'] == $item['talla_id']) {
                            ...
                            @endphp
                            <tr class="fila-carrito-{{ $index }} {{ !$stockSuficiente ? 'table-danger' : '' }}" data-index="{{ $index }}">
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <button class="btn btn-sm btn-outline-secondary btn-cambiar-talla" 
                                                ...
                                                title="Talla anterior">
                                            <i class="fas fa-chevron-left"></i>
                                        </button>
                                        
                                        <!-- Talla Actual -->
                                        <span class="badge bg-primary talla-badge-{{ $index }}" style="font-size: 1rem; padding: 0.5rem 0.75rem;">
                                            {{ $talla->descripcion }}
                                        </span>
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <!-- Cantidad Actual -->
                                        <span class="badge cantidad-badge-{{ $index }} {{ $stockSuficiente ? 'bg-success' : 'bg-danger' }}" 
                                              style="font-size: 1rem; padding: 0.5rem 0.75rem;"
                                              data-stock="{{ $stockTalla }}"
                                              data-precio="{{ $producto->precioP }}"
                                        ...
                                        <!-- Botón Aumentar -->
                                        <button class="btn btn-sm btn-outline-success btn-actualizar-cantidad btn-aumentar-{{ $index }}" 
                                                data-index="{{ $index }}" 
                                ...
                                <td class="td-stock-{{ $index }}">
                                    <div>
                                        ...
                                        <br>
                                        <span class="badge stock-disponible-badge-{{ $index }} {{ $stockDisponible >= 0 ? 'bg-info' : 'bg-danger' }}" title="Stock después de este pedido">
                                            Quedarán: <span class="stock-disponible-valor-{{ $index }}">{{ $stockDisponible }}</span>
                                        </span>
                                    </div>
                                </td>
                                <td>S/. {{ number_format($producto->precioP, 2) }}</td>
                                <td class="td-subtotal-{{ $index }}">S/. <span class="subtotal-valor-{{ $index }}">{{ number_format($subtotal, 2) }}</span></td>
                                <td>
                                    <div class="d-flex gap-1">
                                        ...
                                        @endphp
                                        <button type="button" 
                                                class="btn btn-info btn-sm btn-duplicar-item" 
                                                data-index="{{ $index }}"
        ...
        @if($hayProblemasStock)
            <div class="alert alert-warning mt-3">
                <i class="fas fa-exclamation-triangle me-2"></i>
                <strong>Atención:</strong> Algunos productos en tu carrito no tienen stock suficiente. Por favor, ajusta las cantidades o remueve los productos antes de continuar.
            </div>
        @endif

        <div class="mt-3">
            <a href="{{ route('ventas.create') }}" class="btn btn-success btn-lg {{ $hayProblemasStock ? 'disabled' : '' }}">
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    const btnDisminuir = $(`.btn-actualizar-cantidad[data-index="${index}"][data-accion="disminuir"]`);
                    const btnAumentar = $(`.btn-actualizar-cantidad[data-index="${index}"][data-accion="aumentar"]`);
                    ...
                    badgeCantidad.addClass(stockSuficiente ? 'bg-success' : 'bg-danger');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $(`.td-stock-${index} .badge.bg-secondary`).text(`Stock total: ${response.stock}`);
