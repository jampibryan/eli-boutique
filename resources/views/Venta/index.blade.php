                    ...
                    <tr>
                        ...
                        <td class="align-middle">
                            @if($estadoDescripcion == 'Pendiente')
                                ...
                                <form action="{{ route('ventas.anular', $venta->id) }}" method="POST" style="display:inline;" onsubmit="return confirmAnular();">
                                    @csrf
                                    <button type="submit" class="btn btn-danger btn-sm mt-1">Anular</button>
                                </form>
                            @elseif($estadoDescripcion == 'Pagado')
                                <a href="{{ route('ventas.edit', $venta) }}" class="btn btn-success btn-sm">
...
@section('js')
    ...
    <script>
        function confirmAnular() {
            return confirm('¿Estás seguro de que deseas anular esta venta?');
        }
    </script>
